...
for CPU in "${CPU_COUNTS[@]}"; do
    ...
    sbatch --job-name="MG_${TAG}" \
           ...
           "${SCRIPT_DIR}/test/job_ours_multigrid.sbatch" $NY $NX $NZ $OUT_DIR
    ...
    sbatch --job-name="JAC_${TAG}" \
           ...
           "${SCRIPT_DIR}/test/job_ours_jacobi.sbatch" $NY $NX $NZ $OUT_DIR
    ...
    sbatch --job-name="MAT_${TAG}" \
           ...
           "${SCRIPT_DIR}/test/job_matlab.sbatch" $NY $NX $NZ $OUT_DIR
done
