...
namespace top3d
{
	namespace mg
	{
								 ...
								 const std::vector<double> &xc, std::vector<double> &xf)
		{
			...
			auto idxF = [&](int ix, int iy, int iz)
			{ return fnnx * fnny * iz + fnnx * iy + ix; };
			auto idxC = [&](int ix, int iy, int iz)
...
#pragma omp atomic
							   ...
							   const std::vector<double> &rf, std::vector<double> &rc)
		{
			...
			// Parallel coarse-element accumulation; protect shared node accumulators with atomics
#pragma omp parallel for collapse(3) schedule(static)
			for (int cez = 0; cez < Lc.resZ; ++cez)
			{
				for (int cex = 0; cex < Lc.resX; ++cex)
