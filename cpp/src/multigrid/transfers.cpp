...
void MG_Prolongate_nodes(const MGLevel& Lc, const MGLevel& Lf,
	...
	const int nCoarseElems = fnnx*fnny*fnnz;
	const int minCoarseElemsForParallel = 512; // Need to tune

	xf.assign(nCoarseElems, 0.0);
	...
	#ifdef _OPENMP
		bool doParallel = (nCoarseElems >= minCoarseElemsForParallel) && (omp_get_max_threads() > 1);
		#pragma omp parallel for collapse(3) if (doParallel) schedule(static, chunk)
	#endif
