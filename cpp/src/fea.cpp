                      ...
                      KTimesUWorkspace& ws)
{
	...
	ws.resize_thread_local(nthreads, numFreeDofs);

	#pragma omp parallel
	{
		const int tid = omp_get_thread_num();
		auto& y_local = ws.y_thread_local[tid];
		...
		for (int e_start = 0; e_start < numElements; e_start += chunk_size) {
			...
			// Step 3: Scale and Scatter to thread-local storage
			for (int i = 0; i < num_in_chunk; ++i) {
				const int e = e_start + i;
