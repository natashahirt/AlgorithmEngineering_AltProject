...
namespace top3d
{
				 ...
				 std::vector<double> *freeTmp)
	{
		...
		if ((int)X.size() != pb.mesh.numDOFs)
			X.assign(pb.mesh.numDOFs, 0.0);
		if ((int)Y.size() != pb.mesh.numDOFs)
			Y.assign(pb.mesh.numDOFs, 0.0);
		if ((int)P.size() != pb.mesh.numDOFs)
			P.assign(pb.mesh.numDOFs, 0.0);
		if ((int)A.size() != pb.mesh.numDOFs)
			A.assign(pb.mesh.numDOFs, 0.0);
		if ((int)F.size() != (int)pb.freeDofIndex.size())
			F.assign(pb.freeDofIndex.size(), 0.0);
		...
		double normb_sq = 0.0;
		#ifdef _OPENMP
		#pragma omp parallel for reduction(+ : normb_sq) schedule(static)
		#endif
		for (size_t i = 0; i < bFree.size(); ++i)
			normb_sq += bFree[i] * bFree[i];
		const double normb = std::sqrt(normb_sq);
